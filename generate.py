...
def container(project: str, tag: str, package: str, config):
    ...
    with open('build.sh', 'w') as sh:
        ...
        print(f"buildah build --manifest=ghcr.io/cynix/{container}:{release.tag_name} --network=host --platform={platform} --pull=always .", file=sh)
        print(f"buildah manifest push --all ghcr.io/cynix/{container}:{release.tag_name}", file=sh)
        print(f"buildah manifest push --all ghcr.io/cynix/{container}:{release.tag_name} ghcr.io/cynix/{container}:latest", file=sh)
